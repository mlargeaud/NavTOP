...
pos = [];
pos(1, :, :) = [prop_res(:, 14), prop_res(:, 15)*180/pi, ...
    prop_res(:, 16)*180/pi];

fin_orb_index = size(prop_res, 1);
plotting_functions.plot_wdata(pos(1,:,2)', pos(1,:,3)', ...
    prop_res(fin_orb_index,17), prop_res(fin_orb_index,18), ...
    prop_res(fin_orb_index,19), prop_res(fin_orb_index, 30:35)', 'Hours', ...
    prop_res(:, 1)/3600, ...
    prop_res(:, 29), '', '~');
