...
pos = [];
pos(1, :, :) = [prop_res(:, 8), prop_res(:, 9)*180/pi, ...
    prop_res(:, 10)*180/pi];

fin_orb_index = size(prop_res, 1);
plotting_functions.plot_wdata(pos(1,:,2)', pos(1,:,3)', ...
    prop_res(fin_orb_index,11), prop_res(fin_orb_index,12), ...
    prop_res(fin_orb_index,13), prop_res(fin_orb_index, 24:29)', 'Hours', ...
    prop_res(:, 1)/3600, ...
    prop_res(:, 23), '', '~');
